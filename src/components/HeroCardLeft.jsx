...
let HeroCardLeft = ({ article }) => {
    return (
        <Link to={article?.url} target="blank" className="group block w-full h-full">
            ...
            <div className='max-w-max absolute bottom-5 md:bottom-10 left-5 right-5'>
                <div className="max-w-max text-sm text-white py-1 px-2 rounded-sm bg-gradient-to-r from-rose-500 via-red-400 to-red-500">
                    {article?.source}
                </div>
                <h3 className="text-white py-2 md:py-3 group-hover:underline group-hover:underline-offset-4 group-hover:decoration-secondary">
                    {article?.title}
                </h3>
...
HeroCardLeft = React.memo(HeroCardLeft);

export default HeroCardLeft;

HeroCardLeft.propTypes = {
